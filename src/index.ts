import { McpServer, ResourceTemplate, type ListResourcesCallback } from "@modelcontextprotocol/sdk/server/mcp.js";
import { StdioServerTransport } from "@modelcontextprotocol/sdk/server/stdio.js";
import { z } from "zod";
import { DirectusClient, DirectusConfigSchema } from "./directus-client.js";
...
server.resource(
  "items",
  new ResourceTemplate("items://{collection}/{id?}", { list: listCallback }),
  async (uri, params) => {
    ...
    if (id) {
      ...
      const itemsResponse = await directusClient.listItems(collection) as { data: Record<string, any>[] };
      return {
...
const schemaListCallback: ListResourcesCallback = async () => {
  return {
    resources: [{
...
server.resource(
  "schema",
  new ResourceTemplate("schema://snapshot", { list: schemaListCallback }),
  async (uri) => {
